	 ...
	 */
	 public LoginUser login(String username, String pwd) {
		 ...
		 if(!user.getUserName().equalsIgnoreCase("admin")) {
			 ...
			 //查询结构是否可用
			 R<cn.staitech.system.api.model.OrganizationRes> organizationSelectRes = remoteUserService.getOrganizationById(organizationId+"", SecurityConstants.INNER);
			 //远程接口调用异常
			 if (R.FAIL == organizationSelectRes.getCode()) {
				 throw new ServiceException(organizationSelectRes.getMsg());
			 }
			 //机构数据
			 cn.staitech.system.api.model.OrganizationRes resData = organizationSelectRes.getData();
			 ...
			 //查询结构是否过期
			 R<cn.staitech.system.api.model.OrganizationAuthorization> organizationResult = remoteUserService.selectByOrganizationId(organizationId+"", SecurityConstants.INNER);
			 //远程接口调用异常
			 if (R.FAIL == organizationResult.getCode()) {
				 throw new ServiceException(organizationResult.getMsg());
			 }
			 //机构数据
			 cn.staitech.system.api.model.OrganizationAuthorization organizationAuthorization = organizationResult.getData();
